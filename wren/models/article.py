...
class Article():
    ...
    def __init__(self, article_headline, article_url, **kwargs):
        ...
        """
        self.article_headline = article_headline.encode("utf-8") if article_headline is not None else article_headline
        self.article_url = article_url.encode("utf-8") if article_url is not None else article_url
        self.pub_date = kwargs.get("pub_date", EMPTY_STR)
        self.summary = kwargs.get("summary", EMPTY_STR).encode("utf-8")
        self.article_content = kwargs.get("article_content", EMPTY_STR).encode("utf-8")
        self.article_html = kwargs.get("article_html", EMPTY_STR).encode("utf-8")
        ...
        self.category = kwargs.get("category", EMPTY_STR).encode("utf-8")
        self.language = kwargs.get("language", EMPTY_STR).encode("utf-8")
        self.concepts = {}
        self.media_org = kwargs.get("media_org", EMPTY_STR).encode("utf-8")
